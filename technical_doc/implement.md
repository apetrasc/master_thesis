...
私よりも一つ上の代での共同研究の体制では、実機データをそのまま機械学習に通すというアプローチが採用されていた。そして、評価指標・目標についての共有は不十分であった。例えば、技術開発の目標として相対誤差が求められているのか、絶対誤差が求められているのかが不明瞭であり、またさらに、データがあまりに少ないために、Grad-cam/saliency-mapsの画像をもとに有益な解釈や結論を得るのが難しいという欠点もあった。それらに対し、一般に機械学習の世界での経験則として「前処理に注力することで精度が向上する」というものがあるため、ヒルベルト変換・バンドパスフィルタの適用という思考に至ったと考えられる。
...
総じて、今後はシミュレーションによってデータを生成し、コードと生成結果を厳密にチェックし、それらをパスした場合のみ学習させるという開発体制が考えられる。そこで、テストは厳密かつ詳細に設計されなければならない。今回も、結果として大きな問題ではないことが示されたものの、いくつかのエラーが示された。今後小さなヒューマンエラーが結果を大きく覆すような事態を引き起こす可能性も十分ある。そこで、実施が望まれるテストを各フェーズに応じて提案する。その内容を以下に示す。そして、各フェーズは十分検証されねばならない。
...
#### 2025/11/21
現時点における問題点として、固液二相のシミュレーションが完璧ではないというものが挙げられる。減衰係数を設定する際の周波数のべき乗の関係及び、物性値としての減衰係数、さらには物性値の検証、トランスデューサの設定、構造を数値計算と相違ないように確認する必要がある。今回、フーリエ解析によって得られた情報に関して、振幅の比でもって比較を行い妥当性の検証を行ったのであるが、菅壁の強度の比といった情報に関しても比較・検証を行うべきであった。
特に、これらの物性値は可能であれば実機実験から確認しておくことが望ましいだろう。これらにかかる費用に関しても、ある素材と、基礎試験のみによって実験が行えるだろうし、それらに対して手元で簡単にヒルベルト変換を行って時間差を計測することによって情報の二重チェックが可能となる。例えば実機実験のデータが出せた場合には、少なくともパイプの材質、物性に関し誤っていないことが立証されるだろう。
それさえ正しければ、後は数値計算とトランスデューサの扱いに長けた人物を雇用すれば、シミュレーションの設定に誤りのない再現が可能になる。私のコードを簡単に実行することによって、設計したシミュレータがどの程度正しいのか、数値計算結果がどれほど信頼のおけるものかが評価できるだろう。そして、これらを開発の際に用いることは、実験の検証として役に立つだろう。少なくとも桁があっていれば、実験の際のヒューマンエラーがないことで安心できるだろう。

しかし実験データとしては、数秒単位のスケールでしか得られず、シミュレーションの時間スケールと大きく異なる。これらはカメラ画像と比較するのが良いだろう。ハイスピードカメラによって定点計測を行い、画像処理を行って画像中に含まれるガラスビーズの個数を図ることは、特にビーズが少ない領域において良好に機能する。
さらに、私の固相配置アルゴリズムを適用する場合には、そのアルゴリズムの検証が重要となる。私のアプローチでは、計算系の限界を考慮していなかったがために固相粒子同士がオーバーラップしてしまい信頼性を損ねる結果となった。2025年時点においては比較的性能の高いPCであったが、数値計算の理論限界から鑑みるにより多くのメモリを必要とするだろう。その計算環境を用意するのも重要である。ここでは、私はレンタルする案を推奨したい。研究室のリソースとして契約するのは確かにメリットも多いが、計算機はすぐに陳腐化するので利用が多くなければ無用の長物となる。管理する手間も大きいので、計算機は外注するのを勧める
...
ただ、それでなお使用したいのであれば、何と何の関係をモデル化したいのかを検証するのは重要だろう。
# ソフトウェアの仕様、選定について
